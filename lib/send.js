...
class Send {
	...
	dungeonEvent(message, spcc, type) {
		// If streamer mode is enabled send message to the proxy-channel
		if (this.__deps.mod.settings.stream)
			return this.__deps.mod.command.message(spcc + this.__deps.functions.formatMessage(message));
		...
		this.__deps.mod.send(...this.__deps.proto.getData("S_DUNGEON_EVENT_MESSAGE"), {
			...
			"chat": 0,
			"channel": 27,
			"message": spcc + this.__deps.functions.formatMessage(message)
		});
	}
	 ...
	 * @param {string} [key=null] Original English key for cross-language TTS lookup.
	 * @memberof Send
	 */
	voice(message, enabled = false, key = null) {
		if (!this.__deps.mod.settings.speech.enabled && !this.__deps.mod.settings.speaks && !enabled) return;

		const text = this.__deps.functions.stripTags(this.__deps.functions.formatTags(message));
		// Get current dungeon ID for cross-language TTS lookup
		const dungeonId = this.__deps.zone.loaded ? String(this.__deps.zone.id) : null;
		this.__deps.speech.play(text, null, key, dungeonId);
	}
