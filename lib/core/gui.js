	...
	show(section, pageNumber = 1) {
		...
		switch (section) {
			...
			case "index":
				...
				// Online TTS settings
				if (onlineTTS) {
					const onlineTTSConfig = onlineTTS.getConfig();
					
					// Online TTS switch
					tmpData.push(
						{ "text": `<font color="${gcy}" size="+20">${str.onlineTTS}:</font>` }, { "text": "&#09;&#09;" },
						{ "text": `<font color="${onlineTTSConfig.enabled ? gcg : gcr}" size="+18">[${str.onlineTTSEnable}]</font>`, "command": `${cmd} onlinetts;${cmd} gui` },
						{ "text": `&nbsp;&nbsp;<font color="${gcb}" size="+18">[${str.onlineTTSTest}]</font>`, "command": onlineTTSConfig.enabled ? `${cmd} onlinetts test;${cmd} gui` : "" },
						{ "text": "<br>" }
					);
					
					// If online TTS is enabled, show more options
					if (onlineTTSConfig.enabled) {
						const voices = onlineTTS.getVoices();
						const defaultVoiceVal = voices[onlineTTSConfig.defaultVoice];
						const defaultVoiceLangTag = (typeof defaultVoiceVal === 'object' && defaultVoiceVal.lang) ? ` [${defaultVoiceVal.lang}]` : "";

						// Voice settings
						tmpData.push(
							{ "text": `<font color="${gcy}" size="+20">${str.onlineTTSVoice}:</font>` }, { "text": "&#09;" },
							{ "text": `<font color="${gcgr}" size="+18">${str.onlineTTSCurrentVoice}${onlineTTSConfig.defaultVoice}${defaultVoiceLangTag}</font>` }, { "text": "&nbsp;&nbsp;" },
							{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSChange}]</font>`, "command": `${cmd} admin show onlinetts-voices` }, 
							{ "text": "<br>" },
							{ "text": `<font color="${gcy}" size="+20">${str.onlineTTSRate}:</font>` }, { "text": "&#09;" }
						);
						
						// Rate options
						[0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 3, 4, 5].forEach(rate => {
							tmpData.push(
								{ "text": `<font color="${Math.abs(onlineTTSConfig.rate - rate) < 0.01 ? gcg : gcr}" size="+18">[${rate}]</font>`, "command": `${cmd} onlinetts rate ${rate};${cmd} gui` }, { "text": "&nbsp;" }
							);
						});
						tmpData.push({ "text": "<br>" });
					} else if (!onlineTTSConfig.apiKey) {
						// API key not set prompt
						tmpData.push(
							{ "text": `<font color="${gcr}" size="+18">${str.onlineTTSApiKeyMissing}${cmd} onlinetts apikey YOUR_KEY</font>` },
							{ "text": "<br>" }
						);
					}
			...
			// Online TTS voice settings page
			case "onlinetts-voices":
				const onlineTTSConfig = onlineTTS ? onlineTTS.getConfig() : null;
				
				if (onlineTTS && onlineTTSConfig) {
					const voices = onlineTTS.getVoices();
					const defaultVoiceVal = voices[onlineTTSConfig.defaultVoice];
					const defaultVoiceLangTag = (typeof defaultVoiceVal === 'object' && defaultVoiceVal.lang) ? ` [${defaultVoiceVal.lang}]` : "";

					// Title and current default voice
					tmpData.push(
						{ "text": `<font color="${gcy}" size="+20">${str.onlineTTSVoiceSettings}</font>` }, { "text": "<br><br>" },
						{ "text": `<font color="${gcy}" size="+18">${str.onlineTTSCurrentDefaultVoice}</font>` },
						{ "text": `<font color="${gcg}" size="+18">${onlineTTSConfig.defaultVoice}${defaultVoiceLangTag}</font>` }, { "text": "<br><br>" },
						{ "text": `<font color="${gcy}" size="+18">${str.onlineTTSAddModifyVoice}</font>` }, { "text": "<br>" },
						{ "text": `<font color="${gcgr}" size="+16">${str.onlineTTSUseCommand}${cmd} onlinetts addvoice &lt;Voice Name&gt; &lt;Voice ID&gt;</font>` }, { "text": "<br><br>" },
						{ "text": `<font color="${gcy}" size="+18">${str.onlineTTSAvailableVoices}</font>` }, { "text": "<br>" }
					);
					
					// Show all available voices
					Object.keys(onlineTTS.getVoices()).forEach(voiceName => {
						const isDefault = onlineTTSConfig.defaultVoice === voiceName;
						const val = onlineTTS.getVoices()[voiceName];
						const id = (typeof val === 'object' && val.id) ? val.id : val;
						const langTag = (typeof val === 'object' && val.lang) ? ` [${val.lang}]` : "";

						tmpData.push(
							{ "text": "&nbsp;&nbsp;&nbsp;&nbsp;" },
							{ "text": `<font color="${isDefault ? gcg : gcr}" size="+18">[${str.onlineTTSSelect}]</font>`, "command": `${cmd} onlinetts voice ${voiceName};${cmd} admin show onlinetts-voices` }, { "text": "&nbsp;&nbsp;" },
							{ "text": `<font color="${isDefault ? gcr : gcb}" size="+18">[${str.onlineTTSDelete}]</font>`, "command": `${cmd} onlinetts deletevoice ${voiceName};${cmd} admin show onlinetts-voices` }, { "text": "&nbsp;&nbsp;" },
							{ "text": `<font color="${gcgr}" size="+18">${voiceName}${langTag}: ${id}</font>` }, { "text": "<br>" }
						);
					});
