...
class Gui {
	...
	show(section, pageNumber = 1) {
		...
		const onlineTTS = speech.getOnlineTTS ? speech.getOnlineTTS() : null;
		...
		switch (section) {
			...
			case "index":
				...
				// OnlineTTS settings
				if (onlineTTS) {
					const cacheConfig = onlineTTS.getConfig();
					const selectedPack = onlineTTS.getSelectedPack ? onlineTTS.getSelectedPack() : null;
					const selectedLabel = selectedPack ? `${selectedPack.voice} [${selectedPack.lang}]` : str.onlineTTSNoSelection;

					tmpData.push(
						{ "text": `<font color="${gcy}" size="+20">${str.onlineTTS}:</font>` }, { "text": "&#09;&#09;" },
						{ "text": `<font color="${cacheConfig.enabled ? gcg : gcr}" size="+18">[${str.onlineTTSEnable}]</font>`, "command": `${cmd} onlinetts;${cmd} gui` },
						{ "text": `&nbsp;&nbsp;<font color="${gcb}" size="+18">[${str.onlineTTSTest}]</font>`, "command": cacheConfig.enabled ? `${cmd} onlinetts test;${cmd} gui` : "" },
						{ "text": "<br>" }
					);

					tmpData.push(
						{ "text": `<font color="${gcy}" size="+20">${str.onlineTTSCurrentPack}</font>` }, { "text": "&#09;" },
						{ "text": `<font color="${gcgr}" size="+18">${selectedLabel}</font>` }, { "text": "&nbsp;&nbsp;" },
						{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSChange}]</font>`, "command": `${cmd} admin show onlinetts-voices` },
						{ "text": "<br><br>" }
					);
				}
			...
			// OnlineTTS voice pack settings page
			case "onlinetts-voices":
				if (onlineTTS) {
					const cacheStatus = onlineTTS.getRemoteCacheStatus ? onlineTTS.getRemoteCacheStatus() : {};
					const cacheConfig = onlineTTS.getConfig ? onlineTTS.getConfig() : (mod.settings.onlineTTS || {});
					const packs = onlineTTS.getCachedRemotePacks ? onlineTTS.getCachedRemotePacks() : [];
					const selectedPack = onlineTTS.getSelectedPack ? onlineTTS.getSelectedPack() : null;
					const selectedKey = selectedPack ? `${selectedPack.lang.toLowerCase()}|${selectedPack.voice.toLowerCase()}` : null;
					const previewLabel = str.onlineTTSPreview;
					const autoUpdateEnabled = !!(mod.settings.onlineTTS && mod.settings.onlineTTS.autoUpdateInstalled);
					const debugEnabled = !!(cacheConfig && cacheConfig.debug);

					tmpData.push(
						{ "text": `<font color="${gcy}" size="+20">${str.onlineTTSVoicePacks}</font>` }, { "text": "&nbsp;&nbsp;" },
						{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSRefresh}]</font>`, "command": `${cmd} onlinetts refresh` },
						{ "text": "&nbsp;&nbsp;" },
						{ "text": `<font color="${autoUpdateEnabled ? gcg : gcr}" size="+18">[${str.onlineTTSAutoUpdate}]</font>`, "command": `${cmd} onlinetts autoupdate;${cmd} admin show onlinetts-voices` },
						{ "text": "&nbsp;&nbsp;" },
						{ "text": `<font color="${debugEnabled ? gcg : gcr}" size="+18">[Debug Log]</font>`, "command": `${cmd} onlinetts debug;${cmd} admin show onlinetts-voices` },
						{ "text": "<br><br>" }
					);

					if (cacheStatus.isFetching) {
						tmpData.push(
							{ "text": `<font color="${gcgr}" size="+16">${str.onlineTTSRefreshing}</font>` },
							{ "text": "<br><br>" }
						);
					}
					...
					if (!packs || packs.length === 0) {
						tmpData.push(
							{ "text": `<font color="${gcgr}" size="+18">${str.onlineTTSNoRemoteData}</font>` },
							{ "text": "<br><br>" }
						);
					} else {
						packs.forEach(pack => {
							const packKey = `${pack.langLower}|${pack.voiceLower}`;
							const isSelected = selectedKey === packKey;
							const statusLabel = pack.installed
								? (pack.updateAvailable ? str.onlineTTSUpdateAvailable : str.onlineTTSInstalled)
								: str.onlineTTSNotInstalled;
							...
							tmpData.push(
								{ "text": `<font color="${gcb}" size="+18">[${previewLabel}]</font>`, "command": `${cmd} onlinetts preview ${pack.lang} ${pack.voice}` },
								{ "text": "&nbsp;&nbsp;" }
							);

							if (pack.installed) {
								tmpData.push(
									{ "text": `<font color="${isSelected ? gcg : gcr}" size="+18">[${str.onlineTTSSelect}]</font>`, "command": `${cmd} onlinetts select ${pack.lang} ${pack.voice};${cmd} admin show onlinetts-voices` },
									{ "text": "&nbsp;&nbsp;" }
								);
								if (pack.updateAvailable) {
									tmpData.push(
										{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSUpdate}]</font>`, "command": `${cmd} onlinetts update ${pack.lang} ${pack.voice}` },
										{ "text": "&nbsp;&nbsp;" }
									);
								}
								tmpData.push(
									{ "text": `<font color="${gcr}" size="+18">[${str.onlineTTSDelete}]</font>`, "command": `${cmd} onlinetts delete ${pack.lang} ${pack.voice};${cmd} admin show onlinetts-voices` },
									{ "text": "&nbsp;&nbsp;" }
								);
							} else {
								tmpData.push(
									{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSDownload}]</font>`, "command": `${cmd} onlinetts download ${pack.lang} ${pack.voice}` },
									{ "text": "&nbsp;&nbsp;" }
								);
							}

							const meta = [];
							if (pack.textCount) meta.push(`${pack.generatedCount}/${pack.textCount}`);
							if (pack.localCount) meta.push(`local:${pack.localCount}`);
							const metaText = meta.length > 0 ? ` (${meta.join(" ")})` : "";

							tmpData.push(
								{ "text": `<font color="${gcgr}" size="+18">${pack.lang} / ${pack.voice} - ${statusLabel}${metaText}</font>` },
								{ "text": "<br>" }
							);
						});
					}
					...
					tmpData.push(
						{ "text": "<br>" },
						{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSReturnToMainMenu}]</font>`, "command": `${cmd} admin show index` },
						{ "text": "<br>" }
					);
				} else {
					tmpData.push(
						{ "text": `<font color="${gcr}" size="+20">${str.onlineTTSNotInitialized}</font>` }, { "text": "<br><br>" },
						{ "text": `<font color="${gcb}" size="+18">[${str.onlineTTSReturnToMainMenu}]</font>`, "command": `${cmd} admin show index` }
					);
				}
				break;
