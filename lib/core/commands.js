			...
			"onlinetts": (arg1, arg2, arg3, arg4) => {
				...
				if (["voice", "defaultvoice"].includes(arg1)) {
					if (!arg2) {
						...
						Object.entries(onlineTTS.getVoices()).forEach(([name, val]) => {
							const id = (typeof val === 'object' && val.id) ? val.id : val;
							const langTag = (typeof val === 'object' && val.lang) ? ` [${val.lang}]` : "";
							mod.command.message(`${cy}${name}${config.defaultVoice === name ? ` (${lang.strings.enabled})` : ""}${langTag}: ${cw}${id}`);
						});
						return;
					}
					...
					deps.speech.updateOnlineTTSSettings({ defaultVoice: arg2, enabled: true });
					return mod.command.message(`${cg}${lang.strings.onlineTTSCurrentDefaultVoice}${cy}${arg2}`);
				}

				// Add/modify voice
				if (["addvoice", "setvoice"].includes(arg1)) {
					if (!arg2 || !arg3) return mod.command.message(`${cr}${lang.strings.onlineTTSUseCommand} guide onlinetts addvoice <voice_name> <voice_id> [lang]`);

					const voiceLang = arg4 || lang.language;
					onlineTTS.setVoice(arg2, arg3, voiceLang);
					deps.speech.updateOnlineTTSSettings({ voices: onlineTTS.getVoices() });
					return mod.command.message(`${cg}${lang.strings.onlineTTSVoice} "${arg2}" ${lang.strings.enabled}, ID: ${cy}${arg3}, Lang: ${voiceLang}`);
				}
				...
				if (["rate", "speed"].includes(arg1)) {
					...
					const newSettings = deps.speech.updateOnlineTTSSettings({ rate: parseFloat(arg2) });
					return mod.command.message(`${cg}${lang.strings.onlineTTSRate}: ${cy}${newSettings.rate}`);
				}
				...
				if (["test"].includes(arg1)) {
					...
					if (!voiceName || !config.voices || !config.voices[voiceName]) {
						return mod.command.message(`${cr}${lang.strings.onlineTTSVoice} "${voiceName || 'none'}" ${lang.strings.disabled}`);
					}

					// Get voice language and use corresponding language's test text
					const voiceConfig = config.voices[voiceName];
					const voiceLang = (typeof voiceConfig === 'object' && voiceConfig.lang) ? voiceConfig.lang.toLowerCase() : null;

					// Get test text from i18n based on voice language
					let testText = arg2;
					if (!testText && voiceLang) {
						// Try to get test text from i18nManager for the voice's language
						const i18nManager = mod.i18nManager;
						if (i18nManager && i18nManager.translations["general"]) {
							const generalTrans = i18nManager.translations["general"];
							if (generalTrans[voiceLang] && generalTrans[voiceLang]["onlineTTSTestText"]) {
								testText = generalTrans[voiceLang]["onlineTTSTestText"];
							}
						}
					}
					// Fall back to English default if not found
					if (!testText) {
						testText = lang.strings.onlineTTSTestText || "This is an online TTS test message";
					}

					mod.command.message(`${cg}${lang.strings.onlineTTSTest}, ${lang.strings.voice}: ${cy}${voiceName} [${voiceLang || 'unknown'}]${cg}, ${lang.strings.test}: ${cw}${testText}`);
