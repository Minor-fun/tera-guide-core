	...
	constructor(deps) {
		...
		mod.command.add(params.command, {
			...
			"onlinetts": (arg1, arg2, arg3) => {
				...
				if (!onlineTTS) return mod.command.message(`${cr}Online TTS system not initialized`);
				...
				if (!arg1) {
					...
					if (!config.enabled && !config.apiKey) 
						mod.command.message(`${cr}Warning: Online TTS is enabled but API key is not set, please use "guide onlinetts apikey YOUR_KEY" to set API key`);
					return;
				}

				// Set API key
				if (["apikey", "key", "api"].includes(arg1)) {
					if (!arg2) return mod.command.message(`${cr}Please provide API key`);
					deps.speech.updateOnlineTTSSettings({ apiKey: arg2, enabled: true });
					return mod.command.message(`${cg}Online TTS API key has been set and Online TTS enabled`);
				}
				...
				if (["voice", "defaultvoice"].includes(arg1)) {
					if (!arg2) {
						...
						Object.entries(onlineTTS.getVoices()).forEach(([name, id]) => 
							mod.command.message(`${cy}${name}${config.defaultVoice === name ? " (Default)" : ""}: ${cw}${id}`));
						return;
					}
					
					if (!config.voices[arg2]) return mod.command.message(`${cr}Voice "${arg2}" does not exist, please add this voice first`);
					if (onlineTTS.setDefaultVoice(arg2) === false) return mod.command.message(`${cr}Failed to set default voice`);
					
					deps.speech.updateOnlineTTSSettings({ defaultVoice: arg2 });
					return mod.command.message(`${cg}Default voice has been set to: ${cy}${arg2}`);
				}
				
				// Add/modify voice
				if (["addvoice", "setvoice"].includes(arg1)) {
					if (!arg2 || !arg3) return mod.command.message(`${cr}Usage: guide onlinetts addvoice <voice_name> <voice_id>`);
					
					onlineTTS.setVoice(arg2, arg3);
					deps.speech.updateOnlineTTSSettings({ voices: onlineTTS.getVoices() });
					return mod.command.message(`${cg}Voice "${arg2}" has been added/modified, ID: ${cy}${arg3}`);
				}
				
				// Delete voice
				if (["deletevoice", "removevoice"].includes(arg1)) {
					if (!arg2) return mod.command.message(`${cr}Usage: guide onlinetts deletevoice <voice_name>`);
					if (!config.voices[arg2]) return mod.command.message(`${cr}Voice "${arg2}" does not exist`);
					if (config.defaultVoice === arg2) return mod.command.message(`${cr}Cannot delete current default voice, please set another voice as default first`);
				...
				if (["rate", "speed"].includes(arg1)) {
					...
					return mod.command.message(`${cg}Online TTS rate has been set to: ${cy}${rate}`);
				}
				
				// Test online TTS
				if (["test"].includes(arg1)) {
					if (!config || !config.enabled) return mod.command.message(`${cr}Online TTS is not enabled`);
					if (!config.apiKey) return mod.command.message(`${cr}API key is not set, please set API key first`);
					
					const testText = arg2 || "This is an Online TTS test message";
					const voiceName = arg3 || config.defaultVoice;
					
					if (!config.voices[voiceName]) return mod.command.message(`${cr}Voice "${voiceName}" does not exist`);
					...
					deps.speech.testOnlineTTS(testText, voiceName).then(success => 
						mod.command.message(`${success ? cg : cr}Online TTS test ${success ? 'successful' : 'failed'}`));
					return;
				}
			}
		});
