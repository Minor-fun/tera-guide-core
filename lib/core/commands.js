	...
	constructor(deps) {
		...
		mod.command.add(params.command, {
			...
			"onlinetts": (arg1, arg2, arg3) => {
				...
				if (!onlineTTS) {
					return mod.command.message(`${cr}Online TTS system not initialized`);
				}
				
				// If no parameter, toggle online TTS enable status
				if (!arg1) {
					onlineTTS.updateConfig('enabled', !onlineTTS.getConfig().enabled, true);
					const config = onlineTTS.getConfig();
					mod.command.message(`${cg}Online TTS: ${cy}${config.enabled ? lang.strings.enabled : lang.strings.disabled}`);
					
					// If online TTS is enabled but no API key is set, prompt the user
					if (config.enabled && !config.apiKey) {
						mod.command.message(`${cr}Warning: Online TTS is enabled but no API key is set, please use "guide onlinetts apikey YOUR_KEY" command to set API key`);
					}
				...
				if (["apikey", "key", "api"].includes(arg1)) {
					if (!arg2) {
						return mod.command.message(`${cr}Please provide API key`);
					}
					
					onlineTTS.setApiKey(arg2);
					onlineTTS.setEnabled(true);
					
					mod.command.message(`${cg}Online TTS API key has been set and Online TTS enabled`);
					return;
				}
				...
				if (["voice", "defaultvoice"].includes(arg1)) {
					const config = onlineTTS.getConfig();
					if (!arg2) {
						mod.command.message(`${cg}Current default voice: ${cy}${config.defaultVoice}`);
						mod.command.message(`${cg}Available voices:`);
						Object.entries(onlineTTS.getVoices()).forEach(([voiceName, voiceId]) => {
							mod.command.message(`${cy}${voiceName}${config.defaultVoice === voiceName ? " (default)" : ""}: ${cw}${voiceId}`);
						});
						return;
					}
					
					if (!config.voices[arg2]) {
						return mod.command.message(`${cr}Voice "${arg2}" does not exist, please add this voice first`);
					}
					
					const result = onlineTTS.setDefaultVoice(arg2);
					if (result === false) {
						return mod.command.message(`${cr}Failed to set default voice`);
					}
					
					mod.command.message(`${cg}Default voice has been set to: ${cy}${arg2}`);
					return;
				}
				...
				if (["addvoice", "setvoice"].includes(arg1)) {
					if (!arg2 || !arg3) {
						return mod.command.message(`${cr}Usage: guide onlinetts addvoice <voice name> <voice ID>`);
					}
					
					onlineTTS.setVoice(arg2, arg3);
					mod.command.message(`${cg}Added/modified voice "${arg2}" with ID: ${cy}${arg3}`);
				...
				if (["deletevoice", "removevoice"].includes(arg1)) {
					if (!arg2) {
						return mod.command.message(`${cr}Usage: guide onlinetts deletevoice <voice name>`);
					}
					...
					if (!config.voices[arg2]) {
						return mod.command.message(`${cr}Voice "${arg2}" does not exist`);
					}
					
					// Check if it's the default voice
					if (config.defaultVoice === arg2) {
						return mod.command.message(`${cr}Cannot delete current default voice, please set another voice as default first`);
					}
					
					const result = onlineTTS.deleteVoice(arg2);
					if (result) {
						mod.command.message(`${cg}Voice "${arg2}" has been deleted`);
					} else {
						mod.command.message(`${cr}Failed to delete voice "${arg2}"`);
					}
				...
				if (["rate", "speed"].includes(arg1)) {
					...
					if (!arg2 || isNaN(parseFloat(arg2))) {
						return mod.command.message(`${cg}Current Online TTS rate: ${cy}${config.rate}${cg}, available range: 0.5-5`);
					}
					
					const rate = onlineTTS.setRate(parseFloat(arg2));
					mod.command.message(`${cg}Online TTS rate has been set to: ${cy}${rate}`);
					return;
				}
				...
				if (["test"].includes(arg1)) {
					...
					if (!config || !config.enabled) {
						return mod.command.message(`${cr}Online TTS is not enabled`);
					}
					
					if (!config.apiKey) {
						return mod.command.message(`${cr}API key not set, please set API key first`);
					}
					
					const testText = arg2 || "This is an online TTS test message";
					const voiceName = arg3 || config.defaultVoice;
					
					if (!config.voices[voiceName]) {
						return mod.command.message(`${cr}Voice "${voiceName}" does not exist`);
					}
					
					mod.command.message(`${cg}Testing online TTS, voice: ${cy}${voiceName}${cg}, text: ${cw}${testText}`);
					
					// Use online TTS to play test text
					deps.speech.testOnlineTTS(testText, voiceName)
						.then(success => {
							if (success) {
								mod.command.message(`${cg}Online TTS test successful`);
							} else {
								mod.command.message(`${cr}Online TTS test failed`);
							}
						});
					return;
				}
				...
				mod.command.message(`${cg}Online TTS command list:`);
				mod.command.message(`${cy}guide onlinetts${cw} - Enable/Disable online TTS`);
				mod.command.message(`${cy}guide onlinetts apikey <key>${cw} - Set API key`);
				mod.command.message(`${cy}guide onlinetts voice${cw} - Show current default voice and all available voices with their IDs`);
				mod.command.message(`${cy}guide onlinetts voice <voice name>${cw} - Set default voice`);
				mod.command.message(`${cy}guide onlinetts addvoice <voice name> <voice ID>${cw} - Add/modify voice`);
				mod.command.message(`${cy}guide onlinetts deletevoice <voice name>${cw} - Delete voice`);
				mod.command.message(`${cy}guide onlinetts rate <rate value>${cw} - Set speech rate (range:0.5-5, default:1)`);
				mod.command.message(`${cy}guide onlinetts test [text] [voice name]${cw} - Test online TTS`);
			}
		});
	}
