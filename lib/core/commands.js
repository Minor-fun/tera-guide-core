...
class Commands {
	...
	constructor(deps) {
		...
		mod.command.add(params.command, {
			...
			"guivoicetest": () => {
				// Always use local TTS for voice rate test (this tests system TTS, not OnlineTTS)
				handlers.send.voice(lang.strings.voicetest, true);
				mod.command.message(`${cg}${lang.strings.voicetest}`);
			},

			// OnlineTTS related commands
			"onlinetts": (arg1, arg2, arg3) => {
				...
				const createProgressReporter = (label) => {
					let lastPercent = -1;
					return (info) => {
						if (!info || !info.total) return;
						const percent = info.percent;
						if (percent === lastPercent) return;
						if (!info.initial && percent % 10 !== 0 && percent !== 100) return;
						lastPercent = percent;
						mod.command.message(`${cg}${lang.strings.onlineTTSDownloading} ${cy}${label} ${cw}${percent}% (${info.completed}/${info.total})`);
					};
				};
				const resolveTestText = (targetLang) => {
					const i18nManager = mod.i18nManager;
					const langKey = String(targetLang || "").toLowerCase();
					if (i18nManager && typeof i18nManager.getTranslation === "function" && langKey) {
						const translated = i18nManager.getTranslation("general", "onlineTTSTestText", langKey);
						if (translated) return translated;
					}
					return lang.strings.onlineTTSTestText || "This is an OnlineTTS test message";
				};
				...
				// Toggle auto update of installed packs
				if (["autoupdate", "auto"].includes(arg1)) {
					const nextValue = !config.autoUpdateInstalled;
					deps.speech.updateOnlineTTSSettings({ autoUpdateInstalled: nextValue });
					mod.command.message(`${cg}${lang.strings.onlineTTSAutoUpdate}: ${cy}${nextValue ? lang.strings.enabled : lang.strings.disabled}`);
					return;
				}

				// Toggle OnlineTTS debug logging (temporary, no i18n)
				if (["debug", "dbg", "log"].includes(arg1)) {
					const nextValue = !config.debug;
					deps.speech.updateOnlineTTSSettings({ debug: nextValue });
					mod.command.message(`${cg}OnlineTTS Debug: ${cy}${nextValue ? lang.strings.enabled : lang.strings.disabled}`);
					return;
				}

				// Refresh remote list
				if (["refresh", "list"].includes(arg1)) {
					mod.command.message(`${cg}${lang.strings.onlineTTSRefreshing}`);
					onlineTTS.refreshRemotePacks(true).then(() => {
						mod.command.message(`${cg}${lang.strings.onlineTTSRefreshed}`);
						gui.show("onlinetts-voices");
					}).catch(err => {
						mod.command.message(`${cr}${lang.strings.onlineTTSRefreshFailed} ${err ? err.message : ''}`);
						gui.show("onlinetts-voices");
					});
					return;
				}
				...
				// Select pack
				if (["select", "use"].includes(arg1)) {
					if (!arg2 || !arg3) {
						return mod.command.message(`${cr}${lang.strings.onlineTTSUseCommand} guide onlinetts select <lang> <voice>`);
					}
					const pack = onlineTTS.resolveInstalledPack(arg2, arg3);
					if (!pack) return mod.command.message(`${cr}${lang.strings.onlineTTSPackNotInstalled}`);

					deps.speech.updateOnlineTTSSettings({
						selectedPack: { lang: pack.lang, voice: pack.voice },
						enabled: true
					});
					return mod.command.message(`${cg}${lang.strings.onlineTTSCurrentPack}${cy}${pack.voice} [${pack.lang}]`);
				}

				// Download or update pack
				if (["download", "install", "update"].includes(arg1)) {
					if (!arg2 || !arg3) {
						return mod.command.message(`${cr}${lang.strings.onlineTTSUseCommand} guide onlinetts download <lang> <voice>`);
					}

					mod.command.message(`${cg}${lang.strings.onlineTTSDownloading} ${cy}${arg2}/${arg3}`);
					onlineTTS.downloadPack(arg2, arg3, { force: false, onProgress: createProgressReporter(`${arg2}/${arg3}`) }).then(() => {
						mod.command.message(`${cg}${lang.strings.onlineTTSDownloadComplete}`);
						gui.show("onlinetts-voices");
					}).catch(err => {
						mod.command.message(`${cr}${lang.strings.onlineTTSDownloadFailed} ${err ? err.message : ''}`);
						gui.show("onlinetts-voices");
					});
					return;
				}

				// Preview pack without downloading full cache
				if (["preview", "listen"].includes(arg1)) {
					if (!arg2 || !arg3) {
						return mod.command.message(`${cr}${lang.strings.onlineTTSUseCommand} guide onlinetts preview <lang> <voice>`);
					}

					const previewText = resolveTestText(arg2);

					mod.command.message(`${cg}${lang.strings.onlineTTSPreviewing} ${cy}${arg2}/${arg3}`);
					onlineTTS.previewPack(arg2, arg3, previewText).then(() => {
						mod.command.message(`${cg}${lang.strings.onlineTTSPreviewComplete}`);
					}).catch(err => {
						mod.command.message(`${cr}${lang.strings.onlineTTSPreviewFailed} ${err ? err.message : ''}`);
					});
					return;
				}

				// Delete pack
				if (["delete", "remove"].includes(arg1)) {
					if (!arg2 || !arg3) {
						return mod.command.message(`${cr}${lang.strings.onlineTTSUseCommand} guide onlinetts delete <lang> <voice>`);
					}
					const ok = onlineTTS.deletePack(arg2, arg3);
					mod.command.message(`${ok ? cg : cr}${lang.strings.onlineTTSDelete} ${ok ? lang.strings.enabled : lang.strings.disabled}`);
					gui.show("onlinetts-voices");
					return;
				}
				...
				if (arg1 === "test") {
					...
					const pack = onlineTTS.getSelectedPack();
					if (!pack) return mod.command.message(`${cr}${lang.strings.onlineTTSNoSelection}`);

					let testText = arg2;
					if (!testText) {
						testText = resolveTestText(pack.lang);
					}

					mod.command.message(`${cg}${lang.strings.onlineTTSTest}, ${lang.strings.voice}: ${cy}${pack.voice} [${pack.lang}]${cg}, ${lang.strings.test}: ${cw}${testText}`);
					deps.speech.testOnlineTTS(testText).then(success =>
