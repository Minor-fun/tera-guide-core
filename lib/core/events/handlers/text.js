...
module.exports.text = (deps, data, event) => {
	...
	if (["message", "alert", "warning", "notification", "speech"].includes(event.sub_type)) {
		...
		if (event.speech) {
			const hasParams = message.__hasParams || false;
			// Get the original English key for cross-language TTS lookup
			// For messages with params, key is null (can't use pre-cached audio, must call API in real-time)
			const messageKey = hasParams ? null : (message.__key || null);
			
			deps.handlers.send.voice(message, false, messageKey);
		}
