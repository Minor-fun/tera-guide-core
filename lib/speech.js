...
class Speech {
	...
	__autoUpdateOnlineTTS(packs) {
		...
		if (this.__autoUpdatePromise) return;
		...
		this.__autoUpdatePromise = (async () => {
			for (const pack of targets) {
				...
				try {
					await this.__onlineTTS.downloadPack(pack.lang, pack.voice, { force: false, onProgress });
	...
	destructor() {
		if (this.__onlineTTS) this.__onlineTTS.cancelDownloads();
		this.stop();
	}
}
